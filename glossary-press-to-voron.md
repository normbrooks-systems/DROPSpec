...
**Stall.** The industry's name for the spring-loaded holder a pen clicks into: a latch preloads the cartridge against fixed datum surfaces, carrying the pen's electrical contacts with it. Gang stalls in a half-inch-stepped stack and you have the industrial 2-inch print head. On production floors this is a purchased steel fixture; DROPSpec's is **designed and printed** — an open, parametric part rather than a catalog article. Nearest analog: a toolhead dock, if docking were the whole job. The telling detail: production floors don't actually *say* "stall" — operators just say "pen 1" through "pen 4," because the pens are what you touch and swap while the fixture never fails, never drifts, and therefore never needs a name. Infrastructure that works disappears from vocabulary; that linguistic invisibility is decades of registration reliability, audible as silence — and it is the **standard DROPSpec's printed stall must meet**, not a property it inherits: the deliverables register spends an entire validation track (creep, wear, thermal drift) earning the silence steel got for free. DROPSpec inherits the dialect with one ambiguity engineered out: multi-head installations never agreed on whether head 2's first pen is "pen 5" or "head 2, pen 1," so DROPSpec's canon is hierarchical — head N, pen 1–4 — with the single-head machine's "head 1" going politely unsaid. You keep saying "pen 3"; the config file always knew which head you meant.
...
**Wiping.** Physically cleaning the nozzle face. Industrial TIJ operators do this by hand as ritual; DROPSpec's reference design gives the job to a solenoid-actuated silicone blade making a straight pass at the maintenance position — early prototypes wipe manually while that matures — and in either regime the cartridge never leaves its clamp. The blade is deliberately not a consumable: silicone doesn't absorb, ink wipes off wet, and no part number sneaks back into an open supply chain here. Analog: cleaning your nozzle with a brass brush, if skipping it bricked your printer by Monday.

**Service station.** The parking structure implementing all of the above: cap sled, spittoon, wiper module, maintenance position. Analogous in spirit to a toolchanger dock, but its job is keeping fluid alive, not swapping tools.
...
**Extrusion.** Here: T-slotted aluminum framing, always — 2020 profile for the general frame, a beefier 2040 profile for the gantry that slings the head assembly. Nobody in this project extrudes ink. If a DROPSpec document says "extrusion," someone is talking about structure.
...
**Calibration.** You calibrate flow, PID, input shaping — mostly once per machine. This machine is the same, and the spec makes it a promise: the cartridge's nozzle die is located by its own moulded features, so a conforming pen puts its nozzles where the geometry says, and **swapping a cartridge is not a calibration event** — the spec forbids a machine from requiring one. What gets calibrated is the *machine*: a printed pattern captures each slot's as-built offset once, a property of the gang, stored and done. The print you may see at insertion is a nozzle *health* check — a fast dead-jet arbiter — not an alignment page; the consumer-printer ritual of aligning every new cart is precisely the inherited behavior DROPSpec declines. (Honesty note: the mould-owns-registration claim is the spec's one flagged untested assertion, with a designated swap-and-reprint experiment as its arbiter. If it falsifies, this entry gets rewritten and the door reopens.) Same lifecycle shape as input shaping: per machine, not per consumable.
