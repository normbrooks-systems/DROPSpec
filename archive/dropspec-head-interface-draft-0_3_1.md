...
**Draft 0.3.1 — pre-release working draft**

Part of DROPSpec — the Deposition Raster Open Print Standard.

---

### Changes from 0.3 (non-normative)

Corrections from the 0.3 source-check against the drawings, US 6,332,677 B1, drafts 0.1/0.2.2, and the deliverable register. This summary does not govern; the sections do.

1. §5.3's design note and §5.6 corrected: rule 1 excludes TOOMAH-bearing heads from plain stalls; it never excluded Personality 1 heads from TOOMAH-equipped stalls, and §2 forbids that reading. The case it was misapplied to needs no rule: the block's 5 mm proudness is a standoff, so a carrier advanced against a flat-faced head ends its stroke in free air, 5 mm short of the shell, touching nothing — with vH dead by rule 3 regardless.
2. Two stale cross-references repaired ("§9 records the test" → §10; Open Items moved when §9 was inserted).
3. §6 records that early DDP-01 prototypes will likely wipe manually; the head-side obligation is unchanged.
4. §7.1 Zone F re-anchored: "front face" → face AG; "handle datum" → the grip's lower extent, itself unestablished (new open item).
5. §3.4 seating-land chain restated from a single origin; clamping on film outside the pad field explicitly permitted, on the pads forbidden. §5.1 clearance list corrected (23.35 → 23.25) and tied to the 47.85 mm clamp band.
6. Tangent and land figures reconstructed from trusted values: tangent at B − J = 45.85, usable AD land 19.05 (were 45.75/18.95, taken from the scan fit's centre). J regraded as a scan fit whose radius is trusted as relational. Highest pad edge regraded constructed (caliper check 38.3). The drawings still carry the superseded figures and need regeneration.
7. §5.5 proximity note corrected: the requisitioned set spans the top 9.0 mm of the array (was 7.2).
8. Appendix A.3's duplicate dimension table removed; §4.3 is the single normative home. A.2 notes the provisional Figure A-1 (eight-position outlines) is superseded.
9. §0.1 orphan term fixed (Locked → Stable); §3.3 now states the 0.1 mm allowance §3.2 cites, and separates free growth (AA, AG) from revision-gated growth (Z).
10. New open items: grip lower extent (Zone F); EXC-1 projection versus §3.1 flatness; lowering the TOOMAH narrows the clamp band.
...
**Seating land availability:** AB's flat runs from AC to the fillet tangent at 77.65 (§3.1). Of it, the band from AC to AD level (0 to 13.8) lies in the no-load zone (§3.6), and the TOOMAH occupies 61.65 to 76.65 with a further 1.0 clearance to the tangent (§5.1). The X-clamp band is therefore **13.8 to 61.65 — 47.85 mm**. The pad field spans 14.5 to 38.4 within that band (§4.3): the **contact pads SHALL NOT receive clamping load**; film area outside the pad field MAY serve as clamp land. AD's flat land is 19.05 mm (§3.2). Three of the four profile corners are R14.25; radius-on-radius contact slides rather than locates, so clamps SHALL bear on the flats and not on the corner between them.
...
Resulting clearances on AB: 19.25 to the film top edge, 23.25 to the topmost pad, 3.40 to each edge of Z, and 61.65 mm of AB below the block — of which 47.85 mm lies above the no-load zone and is available to the X clamp (§3.4).
...
**Design note (non-normative):** rule 1 does not address a Personality 1 head in a TOOMAH-equipped stall, and nothing needs to — §2 requires such a head be accepted in every slot, and it seats normally. An erroneous carrier advance against it is a null operation by construction: the receptacles the blades are sized to enter live inside the block, 5 mm proud of AB, so with no block present the blade tips end their stroke in free air, 5 mm short of the shell, contacting nothing — with vH dead by rule 3 regardless. The 5 mm proudness is therefore a standoff as well as a key. The property belongs to the carrier's stroke: a carrier whose travel could push blade tips past the receptacle entry plane forfeits it. Do not build one.
...
- Whether the block's 1.0 mm clearance below the fillet tangent should increase — it strands AB too narrow to clamp on, and moulding tolerance on the tangent could put a flat-bottomed boss onto the start of the curve; note that lowering the block also narrows the §3.4 clamp band (§5.1)
- Whether rule 3 plus the 5 mm standoff fully replace the deleted shutter — believed yes; held open until the carrier design confirms its stroke cannot carry blade tips past the receptacle entry plane (§5.3)
...
The provisional Figure A-1 circulated alongside the 0.2.x drafts carries that revision's eight-position requisition outlines and is superseded by this section, §4.3, and §5.5.
...
*Draft 0.3.1. Geometry gospel; contents wild west. Comments, measurements, and dissent welcome via the project repository.*
